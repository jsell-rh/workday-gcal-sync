import { defineConfig } from 'wxt';

export default defineConfig({
  srcDir: 'src',
  manifest: ({ browser }) => ({
    key: '',
    name: 'PTO Sync - Workday to Google Calendar',
    description: 'Syncs PTO/time-off from Workday to Google Calendar',
    permissions: [
      'identity',
      'storage',
      'activeTab',
      'tabs',
      // sidePanel is Chrome-only; omit for Firefox
      ...(browser === 'firefox' ? [] : ['sidePanel']),
      'alarms',
      'notifications',
    ],
    host_permissions: [
      'https://*.myworkday.com/*',
      'https://www.googleapis.com/*',
      // Firefox needs the identity redirect URL in host_permissions
      ...(browser === 'firefox' ? ['https://*.extensions.allizom.org/*'] : []),
    ],
    // oauth2 is Chrome-only; Firefox uses browser.identity.launchWebAuthFlow
    ...(browser === 'firefox'
      ? {}
      : {
          oauth2: {
            client_id: '5968968327-njl8ho7kcp1876frdljpspl72h8es7f2.apps.googleusercontent.com',
            scopes: [
              'https://www.googleapis.com/auth/calendar.events',
              'https://www.googleapis.com/auth/calendar.readonly',
            ],
          },
        }),
    // Firefox requires browser_specific_settings for AMO
    ...(browser === 'firefox'
      ? {
          browser_specific_settings: {
            gecko: {
              id: 'pto-sync@example.com',
              strict_min_version: '109.0',
            },
          },
        }
      : {}),
  }),
});
